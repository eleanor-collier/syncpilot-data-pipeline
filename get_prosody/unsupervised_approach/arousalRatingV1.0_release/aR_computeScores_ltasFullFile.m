% aR_computeScores_ltasFullFile.m - build neutral emotion models (or global
% if no neutral exists) and score each instance for HF500.
%
% Syntax: 
%   aR_computeScores_ltasFullFile(main,wav_dir,feat_dir,score_dir)   
%
% Subfunctions: 
%   See also: 
%
...
function aR_computeScores_ltasFullFile(main,wav_dir,feat_dir,score_dir,filename)
...
for subject=1:1:length(subjects)
    ...
    for iii=1:1:length(files_neutral)
        ...
        [tok,rem]=strtok(fliplr(Files(files_neutral(iii)).fname),'_');
        file_name=[wav_dir,fliplr(rem(2:end)),'.wav'];
        system(['sox  ',file_name,' ',filename,' trim ',main{files_neutral(iii)}{6},' ',num2str(str2num(main{files_neutral(iii)}{7})-str2num(main{files_neutral(iii)}{6}))]);
        
        [acoustic,Fs]=wavread(filename);
...
for ii=1:1:length(Files)
    ...
    [tok,rem]=strtok(fliplr(Files(ii).fname),'_');
    file_name=[wav_dir,fliplr(rem(2:end)),'.wav'];
    system(['sox  ',file_name,' ',filename,' trim ',main{ii}{6},' ',num2str(str2num(main{ii}{7})-str2num(main{ii}{6}))]);
